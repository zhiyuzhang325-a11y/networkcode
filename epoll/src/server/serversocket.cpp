...
ServerSocket::ServerSocket():sockfd_(socket(AF_INET,SOCK_STREAM,0)) {
    if(sockfd_ < 0) {
...
int ServerSocket::accept() {
    ...
    int conn_fd = ::accept(sockfd_, reinterpret_cast<sockaddr *>(&client_addr), &client_len);
    if(conn_fd < 0) {
...
std::vector<char> ServerSocket::recv(int conn_fd, size_t buf_size) {
    std::vector<char> buf(buf_size);
    ssize_t received = ::recv(conn_fd, buf.data(), buf_size, 0);
    if(received < 0) {
...
size_t ServerSocket::send(int conn_fd, const std::vector<char> &data) {
    size_t total_sent = 0;
    size_t to_send = data.size();
    const char *data_ptr = data.data();
    while(total_sent < to_send) {
        ssize_t n = ::send(conn_fd, data_ptr + total_sent, to_send - total_sent, 0);
        if(n <= 0) break;
        total_sent += n;
    }
