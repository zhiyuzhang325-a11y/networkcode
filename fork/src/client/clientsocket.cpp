...
ClientSocket::ClientSocket(const std::string &ip, size_t port):ClientSocket() {
    // set_non_blocking();
...
std::vector<char> ClientSocket::recv(size_t buf_size)
{
    ...
    if(received < 0) {
        errorMessage("client recv failed: ");
        return {};
    }
    else if(received == 0) {
        logger.log(LogLevel::WARN, "server disconnect\n");
        return {};
    }
    buf.resize(received);
    return buf;
...
size_t ClientSocket::send(const std::vector<char>& data) {
    ...
    logger.log(LogLevel::DEBUG, "send success!\n");
    return total_sent;
}
